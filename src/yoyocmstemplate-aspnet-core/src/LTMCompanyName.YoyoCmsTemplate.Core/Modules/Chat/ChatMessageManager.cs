...
{
    [AbpAuthorize]
    public class ChatMessageManager : YoyoCmsTemplateDomainServiceBase, IChatMessageManager
    {
        ...
        public async Task SendMessageAsync(UserIdentifier sender, UserIdentifier receiver, string message, string senderTenancyName, string senderUserName, Guid? senderProfilePictureId)
        {
            ...
            var entity = (await _friendshipManager.GetFriendshipOrNullAsync(sender, receiver));

            var friendshipState = entity?.State;

...
export class BarChartComponent implements OnInit {
  ...
  private init(el: HTMLElement) {
    ...
    chart.source(dv, {
      xAxis: {
        type:
          this.chartConf.type === 'date'
            ? this.isGroupType || this.chartConf.allAxis.length === 1
              ? 'timeCat'
              : 'time'
            : '',
    ...
    chart
      ...
      .adjust([
        {
          type: this.isGroupType ? 'dodge' : 'stack',
          marginRatio: 0,
        },
      ]);
    chart.tooltip({
    ...
    chart.legend({
      ...
      containerTpl:
        ...
        '<ul class="g2-legend-list" style="list-style-type:none;max-height:150px;margin:0;padding:0;overflow:auto;"></ul>' +
        '</div>', // 图例容器
      itemTpl:

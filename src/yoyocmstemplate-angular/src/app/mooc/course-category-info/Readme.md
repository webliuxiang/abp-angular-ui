...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.CourseCategoryServiceProxy,
			//
			],
	})
...
```ts
{//	课程分类的菜单按钮
text:'CourseCategory',i18n:'CourseCategory',acl:'Pages.CourseCategory',icon:'iconfont icon-dashboard',link:'/app/main/coursecategoryinfo',sort:99},

```

请注意：以上link的值 '/app/main/coursecategoryinfo') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('CourseCategory', 'Pages.CourseCategory', 'iconfont icon-dashboard', '/app/main/coursecategoryinfo')
...
//不带权限验证
```
{  path: 'coursecategoryinfo', component:CourseCategoryComponent }, 
```

...
```ts
{//	友情链接分类的菜单按钮
text:'BlogrollType',i18n:'BlogrollType',acl:'Pages.BlogrollType',icon:'iconfont icon-dashboard',link:'/app/main/blogrolls',sort:99},

```

请注意：以上link的值 '/app/main/blogrolls') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('BlogrollType', 'Pages.BlogrollType', 'iconfont icon-dashboard', '/app/main/blogrolls')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'blogrolls', component:BlogrollTypeComponent,data: { permission: 'Pages.BlogrollType' }  }, 

```
或
//不带权限验证
```
{  path: 'blogrolls', component:BlogrollTypeComponent }, 
```

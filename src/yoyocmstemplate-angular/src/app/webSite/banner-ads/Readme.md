...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.BannerAdServiceProxy,
			//
			],
	})
...
```ts
{//	轮播图广告的菜单按钮
text:'BannerAd',i18n:'BannerAd',acl:'Pages.BannerAd',icon:'iconfont icon-dashboard',link:'/app/main/bannerads',sort:99},

```

请注意：以上link的值 '/app/main/bannerads') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('BannerAd', 'Pages.BannerAd', 'iconfont icon-dashboard', '/app/main/bannerads')
...
//不带权限验证
```
{  path: 'bannerads', component:BannerAdComponent }, 
```

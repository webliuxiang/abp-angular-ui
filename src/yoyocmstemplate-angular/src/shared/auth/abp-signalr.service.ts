...
@Injectable()
export class AbpSignalrService {
  reconnectTime = 5000;
  tries = 1;
  maxTries = 8;

  constructor(public _zone: NgZone, private _chatService: ChatServiceProxy) {
  }
  ...
  friendsSubject = new Subject<ChatFriendDto[]>();
  /**
   * 当前选择的好友
   */
  _selectedUser: ChatFriendDto = new ChatFriendDto();
  selectUserSubject = new Subject<ChatFriendDto>();
    ...
    return new Promise(function(resolve, reject) {
      if (_self.tries > _self.maxTries) {
        ...
        connection
          ...
          .catch(() => {
            setTimeout(() => {
              _self.reconnectstart(connection).then(resolve);
            }, _self.reconnectTime);
  ...
  onUserStateChanged(friend, state) {
    const self = this;
